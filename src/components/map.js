                        ...
                        <Marker key={camera.id} position={[camera.lat, camera.long]} icon={myIcon}>
                            <Popup>
                                <div className={classes.tooltip}>
                                    <div  >
                                        <img id="thumbImg" style={{display: 'none'}} onLoad={i => i.target.style.display='inline'} onError={i => {i.target.style.display='none'}} src={`http://www.vizzion.com/TrafficCamsService/TrafficCams.ashx?strRequest=GetCameraImage7&intCameraID=${camera.id}&intDesiredWidth=720&intDesiredHeight=480&intDesiredDepth=8&intOptions=0&strPassword=${props.config.apiKey}`}/>
                                    </div>
